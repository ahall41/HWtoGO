...
class Nancy extends PGOrgan {
    ...
    public function import(): void {
        ...
        $corrections=[ // stop => [note => [close, front, middle, rear]]
            16=>[ // Positiv: Montre 8'                
                94=>[0, -200, -200, 0], // sic
                95=>[0, +100, -100, 0],
                96=>[0, +100, 0, 0],
        ...
        foreach($corrections as $stopid=>$keydata) {
            foreach($keydata as $midi=>$adjustments) {
                $rankid=$stopid;
                foreach($adjustments as $adjustment) {
                    $rank=$this->getRank($rankid);
                    if ($rank) {
                        $pipe=$rank->Pipe($midi);
                        if (empty($pipe->PitchTuning))
                            {$pipe->PitchTuning=$adjustment;}
                        else
                            {$pipe->PitchTuning+=$adjustment;}
                    }
                    $rankid+=1000;
